...
def construct_attentivefp_bigraph_from_smiles(smiles, acid_or_base):
...
def build_dataset(dataset_smiles, labels_list, smiles_name, acid_or_base):
    dataset_gnn = []
    failed_molecule = []
    labels = dataset_smiles[labels_list]
    smilesList = dataset_smiles[smiles_name]
    ...
    for i, smiles in enumerate(smilesList):
        ...
        un_smiles = Chem.MolToSmiles(un_mol)
        cano_smiles = Chem.MolToSmiles(mol)
        if cano_smiles == un_smiles:
            charge_type = [0]  # neutral
        else:
            charge_type = [1]

        g_attentivefp, ionizable = construct_attentivefp_bigraph_from_smiles(smiles, acid_or_base) #return g
        if g_attentivefp == 1:
            print('{} is transformed failed!'.format(smiles))
            molecule_number = molecule_number - 1
            failed_molecule.append(smiles)
        elif g_attentivefp == 2:
            print('{} does not have ionizable group!'.format(smiles))
            molecule_number = molecule_number - 1
            failed_molecule.append(smiles)
        else:
            mask = build_mask(labels.loc[i], mask_value=123456)
            molecule = [un_smiles, g_attentivefp, labels.loc[i], mask, charge_type, ionizable]
            dataset_gnn.append(molecule)
            print('{}/{} molecule is transformed! {} is transformed failed!'.format(i + 1, molecule_number,
                                                                                    len(failed_molecule)))
...
def built_data_and_save_for_pka(
    ...
    data_set_gnn = build_dataset(dataset_smiles=data_origin, labels_list=labels_list,
                                 smiles_name=smiles_name, acid_or_base=acid_or_base)

    smiles,  g_attentivefp, labels, mask, charge_type, ionizable = map(list, zip(*data_set_gnn))
    graph_labels = {'labels': torch.tensor(labels),
                    'mask': torch.tensor(mask),
                    'charge': torch.tensor(charge_type),
                    'ionizable': torch.tensor(ionizable)
                    }
...
def load_graph_from_csv_bin_for_pka_random_splited(
    ...
    labels = detailed_information['labels']
    mask = detailed_information['mask']
    charge = detailed_information['charge']  # 0 stand for neutral, 1 stand for charged
    ionizable = detailed_information['ionizable']  # 0 stand for not ionizable, 1 stand for ionizable
    ...
    test_set_all = []
    for i in train_index:
        if charge[i] == 0 and ionizable[i] == 1:
            molecule = [smiles[i], g_attentivefp[i], labels[i], mask[i]]
            train_set.append(molecule)
    ...
    for i in test_index:
        if ionizable[i] == 1:
            molecule = [smiles[i], g_attentivefp[i], labels[i], mask[i]]
            test_set_all.append(molecule)
            if charge[i] == 0:
                test_set_neutral.append(molecule)
    print(len(train_set), len(val_set), len(test_set_neutral), len(test_set_all), task_number)
    return train_set, val_set, test_set_neutral, test_set_all, task_number
