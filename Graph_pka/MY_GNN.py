...
class Graph_pka_Predictor(nn.Module):
    """
    Graph_pka is introduced in
    `Multi-instance learning of graph neural networks for aqueous pKa prediction.
    <https://academic.oup.com/bioinformatics/article/38/3/792/6395352>`
    Parameters
    ----------
    node_feat_size : int
        Size for the input node features.
    ...
    def __init__(self,
                 ...
                 n_tasks=1,
                 dropout=0.):
        super(Graph_pka_Predictor, self).__init__()
        ...
        self.node_predict = nn.Sequential(
            nn.Dropout(dropout),
            nn.Linear(graph_feat_size, n_tasks)
        )

    def forward(self, g, node_feats, edge_feats, acid_or_base, acid_center, base_center):
        ...
        Returns
        -------
        micro_pka: float32 tensor of shape (V, 1)
        macro_pka: float32 tensor of shape (G, 1). G for the number of graphs in a batch.
        """
        node_feats = self.gnn(g, node_feats, edge_feats)
        micro_pka = self.node_predict(node_feats)  # (num_nodes, graph_feat_size) ==> (num_nodes, n_tasks)
        # print(micro_pka.shape)  #(421,1)
        # print(node_mask.shape)  #(421,)
        # g.ndata['pka'] = torch.mul(micro_pka, node_mask)

        if acid_or_base == 'acid':
            for i in range(node_feats.shape[0]):
                if acid_center[i] == 0:
                    micro_pka[i] = float('inf')
            g.ndata['pka_exp'] = torch.pow(10, -micro_pka)
            macro_pka = - torch.log10(dgl.sum_nodes(g, 'pka_exp'))
...
class Meter(object):
    ...
    def rmse(self):
        ...
        for task in range(n_tasks):
            ...
            scores.append(
                np.sqrt(F.mse_loss(task_y_pred, task_y_true).cpu().item()))  # the input for F.mse_loss must be tensor
        return scores
...
def run_a_train_epoch(args, model, data_loader, loss_criterion, optimizer, task_weight=None):
    ...
    for batch_id, batch_data in enumerate(data_loader):
        ...
        acid_center = attentivefp_bg.ndata.pop('acid').unsqueeze(dim=1).float().to(args['device'])
        base_center = attentivefp_bg.ndata.pop('base').unsqueeze(dim=1).float().to(args['device'])
        _, logits = model(attentivefp_bg, attentivefp_node_feats, attentivefp_edge_feats,
                          args['acid_or_base'], acid_center, base_center)
...
def run_an_eval_epoch(args, model, data_loader):
    ...
    with torch.no_grad():
        for batch_id, batch_data in enumerate(data_loader):
            ...
            attentivefp_node_feats = attentivefp_bg.ndata.pop(args['node_data_field']).float().to(args['device'])
            attentivefp_edge_feats = attentivefp_bg.edata.pop(args['edge_data_field']).long().to(args['device'])
            acid_center = attentivefp_bg.ndata.pop('acid').unsqueeze(dim=1).float().to(args['device'])
            base_center = attentivefp_bg.ndata.pop('base').unsqueeze(dim=1).float().to(args['device'])

            logits = model(attentivefp_bg, attentivefp_node_feats, attentivefp_edge_feats, args['acid_or_base'],
                           acid_center, base_center)
            labels = labels.type_as(logits).to(args['device'])
...
def run_an_eval_epoch_detail(args, model, data_loader, out_path=None):
    ...
    with torch.no_grad():
        for batch_id, batch_data in enumerate(data_loader):
            ...
            acid_center = attentivefp_bg.ndata.pop('acid').unsqueeze(dim=1).float().to(args['device'])
            base_center = attentivefp_bg.ndata.pop('base').unsqueeze(dim=1).float().to(args['device'])
            _, logits = model(attentivefp_bg, attentivefp_node_feats, attentivefp_edge_feats, args['acid_or_base'],
                              acid_center, base_center)
